...
### Task 3.5: Fix Negative Correlation Multiplier (CRITICAL)

**Files:**
- Modify: `src/kalshi/crypto-bot.py` (line ~784)
- Test: `tests/test_crypto*.py`

**Context:** CRITICAL BUG — The correlation multiplier formula `corr_mult = 1.0 - 0.3 * (max_corr - 0.5) / 0.5` can produce NEGATIVE values. When `max_corr = 1.0` (perfect correlation), `corr_mult = -0.1`, which flips the sign of position sizing.

**Step 1: Write failing test**

```python
def test_correlation_multiplier_never_negative():
    """Correlation multiplier should be in [0, 1], never negative."""
    for corr in [0.0, 0.3, 0.5, 0.7, 0.9, 1.0]:
        mult = compute_correlation_multiplier(corr)
        assert mult >= 0.0, f"Negative mult at corr={corr}: {mult}"
        assert mult <= 1.0, f"Mult > 1 at corr={corr}: {mult}"
```

**Step 2: Fix with clamp**

```python
corr_mult = max(0.1, 1.0 - 0.3 * (max_corr - 0.5) / 0.5)
```
...
| Correlation multiplier | Can go negative (-0.1) | Clamped [0.1, 1.0] | Unit test |
