...
import json, time, base64, datetime, os, sys, re, traceback, hashlib, logging
import requests
...
LOG_PATH = "/tmp/kalshi-entertainment.log"
PID_PATH = "/tmp/kalshi-entertainment.pid"
...
log = logging.getLogger("entertainment-bot")

# === Config ===
MAX_TRADE_AMOUNT = 5  # $5
MAX_DAILY_TRADES = 10
CONFIDENCE_THRESHOLD = 0.85
SCAN_INTERVAL_MINUTES = 15
ENTERTAINMENT_TICKERS = ["ALBUM", "BOX", "MOVIE", "STREAM", "MUSIC", "BILLBOARD", "OSCAR", "GRAMMY", "SPOTIFY", "FILM", "ENTERTAIN"]
...
def place_trade(ticker, side, price_cents, count, reasoning, confidence):
    ...
    try:
        ...
        log.info(f"✅ Order placed: {count}x {side} @ {price_cents}¢ on {ticker}")
...
def parse_album_sales(html):
    ...
    patterns = [
        ...
        # Table-style: number followed by artist
    ...
    for pattern in patterns:
        ...
        for match in matches:
            ...
            try:
                ...
                if units < 1000:
                    units *= 1000
                if 5000 < units < 5_000_000:  # Sanity check
...
def scrape_box_office():
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            # Parse movie titles and grosses
            ...
            for title, gross in movies[:15]:
                ...
                try:
                    ...
                    if val < 1000:
                        val *= 1_000_000  # Likely in millions
                    if val > 50_000:
    ...
    # Weekend estimates (Sunday/Monday)
    try:
...
def match_and_trade(markets, album_data, box_data):
    ...
    for m in markets:
        ...
        log.info(f"\n  📊 {ticker}: {title}")
        log.info(f"     YES ask: {yes_ask}¢ | NO ask: {no_ask}¢ | Last: {last}¢")
...
def evaluate_album_opportunity(market, album, market_price):
    ...
    if side == "yes" and yes_ask and yes_ask < confidence * 100:
        ...
        if edge > 0.05:
            ...
            reasoning = f"HDD: {artist} at {units/1000:.0f}K > {threshold/1000:.0f}K threshold. YES@{yes_ask}¢, conf={confidence*100:.0f}%"
            log.info(f"\n🐙💰 ALBUM ARBITRAGE: {artist} {units/1000:.0f}K units > {threshold/1000:.0f}K")
            log.info(f"    {ticker} YES@{yes_ask}¢ | edge={edge*100:.1f}% | conf={confidence*100:.0f}%")
        ...
        if edge > 0.05:
            ...
            reasoning = f"HDD: {artist} at {units/1000:.0f}K < {threshold/1000:.0f}K threshold. NO@{no_ask}¢, conf={confidence*100:.0f}%"
            log.info(f"\n🐙💰 ALBUM ARBITRAGE: {artist} {units/1000:.0f}K units < {threshold/1000:.0f}K")
            log.info(f"    {ticker} NO@{no_ask}¢ | edge={edge*100:.1f}% | conf={confidence*100:.0f}%")
...
def evaluate_boxoffice_opportunity(market, movie, market_price):
    ...
    if side == "yes" and yes_ask and yes_ask < confidence * 100:
        ...
        if edge > 0.05:
            count = max(1, min((MAX_TRADE_AMOUNT * 100) // yes_ask, 20))
            reasoning = f"Box office: {movie_title} ${gross/1e6:.1f}M > ${threshold/1e6:.0f}M. YES@{yes_ask}¢, conf={confidence*100:.0f}%"
        ...
        if edge > 0.05:
            count = max(1, min((MAX_TRADE_AMOUNT * 100) // no_ask, 20))
            reasoning = f"Box office: {movie_title} ${gross/1e6:.1f}M < ${threshold/1e6:.0f}M. NO@{no_ask}¢, conf={confidence*100:.0f}%"
            log.info(f"\n🐙💰 BOX OFFICE ARBITRAGE: {movie_title} ${gross/1e6:.1f}M < ${threshold/1e6:.0f}M")
            place_trade(ticker, "no", no_ask, count, reasoning, confidence)
...
def scan():
    ...
    try:
        ...
        log.info(f"Balance: ${bal.get('balance', 0)/100:.2f}")
    except Exception as e:
    ...
    for m in markets[:20]:  # Log first 20
...
def main():
    ...
    try:
        bal = api("GET", "/portfolio/balance")
        log.info(f"✅ Auth OK! Balance: ${bal.get('balance', 0)/100:.2f}")
    except Exception as e:
