...
async def _fetch_market_context(real_client: RealSportsClient, kalshi_client: KalshiClient):
    ...
    kalshi_markets = await _fetch_kalshi_game_markets(kalshi_client)
    return {
        "schedule": schedule,
        "home_feed": home_feed,
...
async def _refresh_loop(
    ...
    while not is_shutdown_requested():
        try:
            ...
            index = probe.refresh_game_market_index(
            ...
            log.info(
                "Latency probe refreshed game index: %d discovery games, %d crowd markets, %d mapped tickers",
...
async def async_main(
    ...
    if kalshi_stream is not None:
        await kalshi_stream.set_market_tickers(_mapped_tickers(index))
    log.info(
        "Latency probe initialized: %d discovery games, %d crowd markets, %d Kalshi markets, %d mapped games",
        len(_mapping_games(context)),
        len(context.get("crowd_markets", [])),
    ...
    baseline_snapshots = await probe.capture_market_index_snapshot(
        extra={
            ...
            "kalshi_markets_count": len(_mapping_kalshi_markets(context)),
        }
    )
    log.info(
