...
PIPELINE_CRON_LINE="0 6 * * * /bin/bash -c \"cd $PROJECT_DIR && python3 scripts/calibration-pipeline.py\" >> $PIPELINE_LOG 2>&1 $PIPELINE_CRON_TAG"

# Get existing crontab (empty string if none)
EXISTING="$(crontab -l 2>/dev/null || true)"

# Remove any existing kalshi-sync and kalshi-pipeline lines
...
echo "     Command:  cd $PROJECT_DIR && python3 scripts/calibration-pipeline.py"
